...
+ 查询电量：进入操作和逻辑中的一致（如果手电处于开机状态，则通过双击+长按进入）。当进入电量查询模式后手电的颈部开关指示灯首先将会以红黄绿依次点亮的方式（**若电池电压低于10.00V，则系统将会切换到高精度显示模式以0.01V精度显示电量。此时先导提示将变为绿黄红的顺序**）作为先导提示，提示用户手电即将开始显示电池电压和电量。如果电池电压在合法范围内，则在延迟1秒后手电将通过红色，黄色和绿色的指定次数闪烁提示用户当前的电池电压。如果电池电压超出显示范围（例如强制接入四串电池）则驱动会通过三次红色闪烁提示电压非法。对于电压显示的计算方式请参考如下内容：
