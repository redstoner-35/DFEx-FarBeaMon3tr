...
+ 战术模式：关机状态下四击开关，手电主灯迅速频闪一次表示进入战术模式。默认情况下战术功能为按下开关手电以极亮运行，松开则熄灭。当手电进入战术模式后，侧部按键将会每2秒快闪一次指示当前处于战术模式，此时手电将会保持唤醒，直到无操作10分钟后自动进入休眠并退出战术模式。与此同时，您可以通过双击侧按切换战术模式的功能，切换后的指示如下：
...
+ 温度查询模式：手电任意状态下三击+长按进入。当进入电量查询模式后手电的颈部开关指示灯首先将会以红黄绿依次点亮的方式（**若手电当前处于极寒环境，手电本身温度低于0摄氏度时，先导提示将变为绿黄红的顺序**）作为先导提示，提示用户手电即将开始显示手电当前的内部温度。延迟1秒后手电将通过红色和黄色的指定次数的慢速闪烁（如果快速闪烁一次，则表示该数据位为0）提示用户当前的手电温度，对于温度显示的计算方式请参考如下内容：
...
播报完当前手电的内部温度数值后，手电将会在延迟2秒后通过后续的长亮(或者慢闪)提示用户当前内部温度数值所对应的手电散热情况，指示方式如下：
    
  + 绿色：当前手电散热良好
  + 黄色：当前手电温度较高，请注意防止烫手
  + 红色：当前手电温度极高，请及时为手电散热并且避免被烫伤 

+ 查询电量：手电任意状态下通过双击+长按进入。当进入电量查询模式后手电的颈部开关指示灯首先将会以红黄绿依次点亮的方式（**若电池电压低于10.00V，则系统将会切换到高精度显示模式以0.01V精度显示电量。此时先导提示将变为绿黄红的顺序**）作为先导提示，提示用户手电即将开始显示电池电压和电量。延迟1秒后手电将通过红色，黄色和绿色的指定次数的慢速闪烁（如果快速闪烁一次，则表示该数据位为0）提示用户当前的电池电压，对于电压显示的计算方式请参考如下内容：
