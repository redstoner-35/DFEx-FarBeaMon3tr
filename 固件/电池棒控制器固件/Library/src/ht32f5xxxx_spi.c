...
/*********************************************************************************************************//**
 * @brief Set Clock Pre-Divider to Program SCLK Frequency of target SPI.
 * @param SPIx: where SPIx is the selected SPI from the SPI peripherals.
 * @param SPI_ClockPrescaler: actual Divide value input,fSCLK=Fsys/SPI_ClockPrescaler.
 * @retval None
 ************************************************************************************************************/
void SPI_SetClockPrescaler(HT_SPI_TypeDef* SPIx, u32 SPI_ClockPrescaler)
{
/* Check the parameters                                                                                   */
Assert_Param(IS_SPI_CLOCK_PRESCALER(SPI_ClockPrescaler));
/*---------------------------- SPIx Clock Prescaler Register Configuration -------------------------------*/
  #if (LIBCFG_SPI_CLK_PRE_V01)
  SPIx->CPR = (SPI_ClockPrescaler - 1);
  #else
  SPIx->CPR = (SPI_ClockPrescaler / 2) - 1;
  #endif
}
